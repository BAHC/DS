...
echo '<p class="color-', $_assertion ,'">forget() ', $_assertion, '</p>';
...
if ($_value !== $_res) {
    echo '<p>value = ', $_value,'; ', $_key,' = ', $_res, '<br /></p>';
    $_assertion = 'OK';
}
...
$DS::preserveCase(true);
if (DEBUG) {
    echo '<p>preserveCase = true</p>';
}

$_assertion = 'FAIL';
$_keys = ['a/e->1'=>'abc', 'a/e->2'=>'def', 'a/e->3'=>'ghi', 'Claus'=>['Santa', 'process'],];
$DS::putMany($_keys);
$_tags = $DS::forget(['a/e->1', 'a/e->2', 'Santa']); //ignored 'Santa' key
$_res = empty($DS::tags($_tags));
if (true === $_res){
    $_assertion = 'OK';
}

dd('Tags: '. implode(', ', $_tags));
dd($_keys);
dd($DS::tags(['a/e', 'Santa']));
